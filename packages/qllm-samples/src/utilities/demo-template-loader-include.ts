...
async function demoTemplateLoader(): Promise<void> {
    ...
    await fs.writeFile(
        path.join(tempDir, file2Path),
        "This is content from file 2 (partial path).",
    );
    await fs.writeFile(
        file4Path,
        "This is content from file 4 (file:// syntax).",
    );
...
`;
    ...
    try {
        ...
        if (
            localResolvedTemplate.content.includes(
                "This is content from file 1 (full path).",
            ) &&
            localResolvedTemplate.content.includes(
                "This is content from file 2 (partial path).",
            ) &&
            localResolvedTemplate.content.includes(
                "Realizing AI: From Concept to Business",
            ) &&
            localResolvedTemplate.content.includes(
                "This is content from file 4 (file:// syntax).",
            )
            ...
            console.log(
                "All file inclusions in the local template were successfully resolved!",
            );
        } else {
            console.log(
                "Some file inclusions in the local template were not resolved correctly.",
            );
        }

        // Load and resolve the template from GitHub

        const githubResolvedTemplate: TemplateDefinitionWithResolvedContent =
        ...
        if (githubResolvedTemplate.content?.includes("using Markdown")) {
            console.log(
                "The story.md file was successfully included in the GitHub template!",
            );
        } else {
            console.log(
                "The story.md file was not included correctly in the GitHub template.",
            );
        }
    } catch (error) {
