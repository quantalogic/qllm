...
const testDocumentLoader = async () => {
    ...
    console.log("\nLoading from URL:");
    const urlLoader = new DocumentLoader(testUrl, {
        ...options,
        decompress: true,
    });
    ...
    urlLoader.on("loaded", (result: LoadResult<Buffer>) => {
        console.log(
            "URL content loaded successfully. Content length:",
            result.content.length,
        );
    ...
    const quickFileResult = await DocumentLoader.quickLoadString(
        `file://${testFullPath}`,
        options,
    );
    ...
    const quickBufferResult = await DocumentLoader.quickLoadBuffer(
        `file://${testFullPath}`,
        options,
    );
    console.log(
        "Quick buffer content type:",
        quickBufferResult.content instanceof Buffer,
    );
    console.log(
        "Quick buffer content length:",
        quickBufferResult.content.length,
    );
    ...
    multipleResults.forEach((result, index) => {
        console.log(
            `Document ${index + 1} loaded. Content length:`,
            result.content.length,
        );
