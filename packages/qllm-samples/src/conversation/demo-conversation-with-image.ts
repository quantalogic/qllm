...
async function main() {
    ...
    const conversation = await conversationManager.createConversation({
        metadata: {
            ...
            description:
                "A conversation about describing an image using OpenAI's vision capabilities",
        },
    ...
    async function chatTurn(userMessage: string, imageUrl?: string) {
        ...
        await conversationManager.addMessage(conversation.id, {
            ...
            content: {
                type: "text",
                text: fullResponse || "Sorry, I couldn't generate a response.",
            },
            providerId: "openai",
        });
    }
    ...
    await chatTurn(
        "Can you describe the lighting and color palette used in the image?",
    );
    ...
    finalHistory.forEach((msg) => {
        console.log(
            `${msg.role.toUpperCase()}: ${displayMessageContent(msg.content)}`,
        );
    });
}
