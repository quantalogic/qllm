...
const simpleGreeting = TemplateDefinitionBuilder.quickSetup(
    "Simple Greeting",
    "Hello, {{name}}!",
)
    .withInputVariable("name", "string", "The name of the person to greet")
    .withOutputVariable("greeting", "string", {
...
const weatherReport = TemplateDefinitionBuilder.create({
    ...
    .withInputVariable(
        "condition",
        "string",
        "The weather condition (e.g., sunny, rainy)",
    )
    .withOutputVariable("report", "string", {
...
const recipeGenerator = TemplateDefinitionBuilder.create({
    ...
    description:
        "Generate a recipe based on ingredients and dietary restrictions", // {{description}}
   ...
   Instructions:
    ...
    .withInputVariable(
        "vegetarian",
        "boolean",
        "Whether the recipe should be vegetarian",
        {
            default: false,
        },
    )
    .withInputVariable(
        "vegan",
        "boolean",
        "Whether the recipe should be vegan",
        { default: false },
    )
    .withOutputVariable("recipe", "string", {
    ...
    .withPromptType("text_generation")
    .withTaskDescription(
        "Generate a recipe based on given ingredients and dietary restrictions",
    )
    .withCustomInputValidator(
        "ingredients",
        (value: string) => value.split(",").length >= 3,
    )
    .withExampleOutputs(
...
const codeGenerator = TemplateDefinitionBuilder.create({
   ...
   // Code goes here
    ...
    .withSystemMessage(
        "You are a pirate. You are a nice pirate. You are a very nice pirate.",
    )
    .build();
...
const interactiveStoryGenerator = TemplateDefinitionBuilder.create({
  ...
  3. [Choice 3]
    ...
    .withInputVariable(
        "current_scene",
        "string",
        "Description of the current scene in the story",
    )
    .withInputVariable(
        "player_choice",
        "string",
    ...
    .withInputVariable(
        "include_dialogue",
        "boolean",
        "Whether to include character dialogue",
        {
            default: true,
        },
    )
    .withInputVariable(
        "add_plot_twist",
        "boolean",
        "Whether to add an unexpected plot twist",
        {
            default: false,
        },
    )
    ...
    .withCustomInputValidator(
        "character_names",
        (value: string) => value.split(",").length >= 2,
    )
    .withExampleOutputs(
...
const modifiedWeatherReport = TemplateDefinitionBuilder.fromJSON(
...
// Merging templates
const mergedTemplate = TemplateDefinitionBuilder.fromJSON(
    JSON.stringify(recipeGenerator),
)
    .merge(TemplateDefinitionBuilder.fromJSON(JSON.stringify(weatherReport)))
    .withTaskDescription(
        "Generate a recipe suitable for the current weather conditions",
    )
    .build();
