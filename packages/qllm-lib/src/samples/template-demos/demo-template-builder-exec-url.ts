...
async function main(): Promise<void> {
  const documentSummarizer = TemplateDefinitionBuilder.create({
    ...
    .withInputVariable('file_url', 'string', '🔗 The URL of the document to summarize')
    .withInputVariable('max_words', 'number', '📏 Maximum number of words for the summary')
    ...
    .withTaskDescription('🎯 Generate concise summaries of documents from provided URLs')
    .build();
...
main()
  ...
  .catch((err) => {
    console.error('❌ An error occurred while running the document summarizer:', err);
  });
