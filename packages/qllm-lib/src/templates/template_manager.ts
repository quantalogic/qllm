...
import { InputValidationError, TemplateDefinition, TemplateError, TemplateVariable } from './types';
  ...
  async getTemplate(name: string): Promise<TemplateDefinition | null> {
    ...
    try {
      ...
      if (!template || typeof template !== 'object') {
        ErrorManager.throw(TemplateError, `Invalid template structure for ${name}`);
      }
