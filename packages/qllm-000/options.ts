// src/options.ts
import { ErrorManager } from 'qllm-lib';
...
export const fileOption = new Option('-f, --file <path>', 'Path to input file').argParser(
  (value) => {
...
export const outputOption = new Option('-o, --output <path>', 'Path to output file').argParser(
  (value) => {
    if (typeof value !== 'string' || value.trim().length === 0) {
...
export const streamOption = new Option('--stream', 'Stream the output').default(false);
