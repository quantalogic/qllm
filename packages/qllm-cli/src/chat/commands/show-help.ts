...
export function showHelp(
  ...
  helpGroups.forEach((group) => {
    ...
    group.commands.forEach((cmd) => {
      ioManager.displayInfo(`  ${ioManager.colorize(cmd.command, "cyan")} - ${cmd.description}`);
    });
    ioManager.newLine();
  });

  ioManager.displayInfo(ioManager.colorize("Tip: Use '/help <command>' for more detailed information about a specific command.", "yellow"));

  return Promise.resolve();
}

function showCommandHelp(command: string, { ioManager }: Pick<CommandContext, 'ioManager'>): Promise<void> {
  const allCommands = helpGroups.flatMap(group => group.commands);
  const commandHelp = allCommands.find(cmd => cmd.command.split(' ')[0] === command);
  ...
  if (commandHelp) {
    ...
    ioManager.displayInfo(`  ${ioManager.colorize(commandHelp.command, "cyan")}`);
