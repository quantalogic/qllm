...
export class AnthropicProvider implements LLMProvider {
    ...
    constructor(credentials: AwsCredentialIdentity, private region: string) {
    ...
    async generateMessage(messages: Message[], options: LLMProviderOptions): Promise<string> {
        try {
            ...
            return response.content && response.content.length > 0 && response.content[0].type === 'text' ? response.content[0].text : '';
        } catch (error) {
